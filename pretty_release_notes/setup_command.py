...
from .openai_client import DEFAULT_MODEL, SUPPORTED_REASONING_EFFORTS, normalize_reasoning_effort
...
def setup_config(
	...
	reasoning_effort = _prompt_reasoning_effort(existing_values.get("reasoning_effort"))
...
def _flatten_toml(toml_config: dict) -> dict:
	...
	flat["reasoning_effort"] = llm.get("reasoning_effort", "")
...
def _migrate_env_to_dict(env_values: dict) -> dict:
	...
	return {
		...
		"reasoning_effort": env_values.get("LLM_REASONING_EFFORT") or env_values.get("OPENAI_REASONING_EFFORT") or "",
...
def _build_toml_content(
	...
	reasoning_effort_line = (
		f'reasoning_effort = "{reasoning_effort}"' if reasoning_effort else '# reasoning_effort = "medium"'
	)
...
# Supported values: {", ".join(SUPPORTED_REASONING_EFFORTS)}
{reasoning_effort_line}
...
def _prompt_reasoning_effort(default: str | None) -> str | None:
	"""Prompt for reasoning effort, allowing blank to keep provider defaults."""
	supported_values = ", ".join(SUPPORTED_REASONING_EFFORTS)

	while True:
		value = Prompt.ask(
			f"Reasoning effort (optional; blank uses provider default: {supported_values})",
			default=default or "",
		)
		try:
